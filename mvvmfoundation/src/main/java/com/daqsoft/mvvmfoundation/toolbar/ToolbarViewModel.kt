...
abstract class ToolbarViewModel<M : BaseModel> : BaseViewModel<M> {
    ...
    //标题文字颜色
    var titleTextColorObservable = ObservableInt(Color.parseColor("#333333"))

    //左边返回按钮图片
    var  backIconSrcObservable = ObservableInt(R.mipmap.notice_detail_back_normal)
    //左边返回按钮隐藏
    var  backIconVisibleObservable =  ObservableInt(View.VISIBLE)

    //右边按钮图片
    var rightIconSrcObservable = ObservableInt(R.mipmap.more_black)
    //右边按钮隐藏
    var rightIconVisibleObservable = ObservableInt(View.GONE)

    //右边按钮图片2
    var rightIcon2SrcObservable = ObservableInt(R.mipmap.more_black)
    //右边按钮隐藏2
    ...
    //右边文字颜色
    var rightTextColorObservable = ObservableInt(Color.parseColor("#333333"))
    //右边文字 DrawableRight
    var rightTextDrawableRightObservable = ObservableInt(R.mipmap.more_black)

    //背景颜色
    var backgroundObservable = ObservableInt(Color.WHITE)
    ...
    // 标题右图标
    var titleRightIconObservable = ObservableInt(R.mipmap.more_black)
    // 标题右图标隐藏
    var titleRightIconVisibleObservable = ObservableInt(View.GONE)

    init {
        statusBarHeight.set(ToolbarUtils.getStatusBarHeight())
        toolbarHeight.set(ToolbarUtils.getStatusBarHeight() + ToolbarUtils.getToolbarHeight())
        Timber.e("getStatusBarHeight" + ToolbarUtils.getStatusBarHeight())
        Timber.e("getToolbarHeight" + ToolbarUtils.getToolbarHeight())
    }
    ...
    /**
     * 设置右边文字 DrawableRight
     */
    fun setRightTextDrawableRight(res: Int){
        rightTextDrawableRightObservable.set(res)
    }
