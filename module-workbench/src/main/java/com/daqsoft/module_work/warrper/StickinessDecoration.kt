...
class StickinessDecoration : RecyclerView.ItemDecoration() {
    /**
     * 吸附的itemView
     */
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        if (!mCurrentUIFindStickView) {
            mStickyItemViewMarginTop = 0
            if (mLayoutManager!!.findFirstVisibleItemPosition() + parent.childCount == parent.adapter!!
                    .itemCount && mStickyPositionList.size > 0
            ) {
                bindDataForStickyView(
